...
const WithdrawManagementList = (props: any): JSX.Element => {
  ...
  const commodityElements = React.useMemo(() => {
    return props.withdraws.map((commodity: any, i: number) => (
      <tr
        className={`table-row`}
        key={commodity._id}
      >
  ...
  }, [props.withdraws, loadingDepositId]);
...
export default WithdrawManagementList;
